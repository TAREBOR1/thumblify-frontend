       ...
       <div className={`relative overflow-hidden ${aspectClass[aspectRatio]}`}>
        {/* Loading state */}
          ...
          {!isLoading && thumbnail?.image_url && (
            ...
            <Image src={thumbnail?.image_url} alt={thumbnail.title} width={100} height={100} className='w-full h-full object-cover'/>
            <div className='absolute inset-0 flex items-end justify-center bg-black/10 opacity-0 transition-opacity group-hover:opacity-100'>
                <button onClick={onDownload} className='mb-6 flex items-center gap-2 rounded-md px-5 py-2.5 text-xs font-medium transition bg-white/30 ring-2 ring-white/40 backdrop-blur-2xl hover:scale-105 active:scale-95' type='button'>
