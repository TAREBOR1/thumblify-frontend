import { ThumbnailStyle, thumbnailStyles } from '@/assets/assets'
...
const StyleSelector = ({value,onChange,isOpen,setIsOpen}:{value:ThumbnailStyle;onChange:(style:ThumbnailStyle)=>void;isOpen:boolean;setIsOpen:(open:boolean)=>void}) => {
    const styleDescription:Record<ThumbnailStyle,string>={
        ...
        "Minimalist" :"Clean, simple, lots of space",
  ...
  return (
    <div className='relative space-y-3 dark'>
      ...
      {
        isOpen && (<div className='absolute bottom-0 z-50 mt-1 w-full rounded-md border border-white/12 bg-black/20 backdrop-blur-3xl shadow-lg'>
            {thumbnailStyles.map((style)=>{
         return <button key={style} type='button' onClick={()=>{onChange(style);setIsOpen(false)}}
         className='flex w-full items-start gap-3 px-4 py-3 text-left transition hover:bg-black/30'>
          <div className='mt-0.5'>
            {styleIcon[style]}
          </div>
          <div><p className='font-medium'>{style}</p>
          <p className='text-xs text-zinc-400'>{styleDescription[style]}</p></div>
         </button>
            })}
            </div>)
      }
